...
Reproducibility: Running fetalfusion_FINAL_SUBMISSION.ipynb end-to-end reproduces the results reported in Table 1–4 of the paper. All baselines (U-Net, Att. U-Net, nnU-Net v2, VM-UNet, U-Mamba) are implemented within the same notebook under identical training conditions: AdamW lr=1e-4, 50 epochs, early stopping patience=10, identical augmentation, same train/val split (seed=42).

Note on TransUNet: Due to Kaggle session time constraints, TransUNet was evaluated in a separate notebook with strictly identical hyperparameters, optimizer, augmentation pipeline, and data splits. The architecture implementation follows the original paper (8 transformer layers, 768-d, 12 heads).
